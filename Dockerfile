...
# Use the official Ruby 3.3.4 Slim Bullseye image as base:
...
RUN userhome=$(eval echo ~${DEVELOPER_USERNAME}) \
    ...
    && mkdir -p /workspaces/nexu-exercise \
    && chown -R ${DEVELOPER_USERNAME}:${DEVELOPER_USERNAME} /workspaces/nexu-exercise

# Add the app's "bin/" directory to PATH:
ENV PATH=/workspaces/nexu-exercise/bin:$PATH

# Set the app path as the working directory:
WORKDIR /workspaces/nexu-exercise
...
COPY --chown=${DEVELOPER_USERNAME} Gemfile* /workspaces/nexu-exercise/
...
COPY --from=testing /workspaces/nexu-exercise/ /workspaces/nexu-exercise/
...
RUN git config --global --add safe.directory /workspaces/nexu-exercise
...
COPY --chown=${DEVELOPER_USERNAME} . /workspaces/nexu-exercise/
...
# final destination at /workspaces/nexu-exercise:
COPY --from=builder --chown=nobody:nogroup /workspaces/nexu-exercise /workspaces/nexu-exercise

# Set the container user to 'nobody':
USER nobody

# Set the RAILS and PORT default values:
ENV HOME=/workspaces/nexu-exercise \
...
ENTRYPOINT [ "/workspaces/nexu-exercise/bin/entrypoint" ]
