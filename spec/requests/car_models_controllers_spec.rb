...
RSpec.describe CarModelsController, type: :request do
  ...
  describe 'GET /brands/:brand_id/models' do
  ...
  describe 'GET /models' do
    ...
    it 'returns car models with a price greater than a specified value' do
      get '/models', params: { greater: 200_000 }
      expect(json).not_to be_empty
      expect(json.size).to eq(1)
      expect(response).to have_http_status(:ok)
    end
    ...
    it 'returns car models with a price lower than a specified value' do
      get '/models', params: { lower: 400_000 }
      expect(json).not_to be_empty
      expect(json.size).to eq(1)
      expect(response).to have_http_status(:ok)
    end
  ...
  describe 'POST /brands/:brand_id/models' do
    ...
    context 'with invalid attributes' do
      it 'returns an error' do
        post "/brands/#{brand.id}/models", params: { car_model: invalid_attributes }
  ...
  describe 'PUT /models/:id' do
    context 'with valid attributes' do
      it 'updates the car model' do
        put "/models/#{car_model.id}", params: { car_model: { average_price: 400_000 } }
        car_model.reload
    ...
    context 'with invalid attributes' do
      it 'returns an error if average_price is below 100,000' do
        put "/models/#{car_model.id}", params: { car_model: { average_price: 90_000 } }
